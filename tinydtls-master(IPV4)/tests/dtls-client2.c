...
int main(int argc, char **argv) {
 ...
 // dst.addr.sin.sin_port = htons(atoi(optind < argc ? argv[optind++] : port_str));
    printf(" Client listen port %d",dst.addr.sin.sin_port);
...
//  dtls_connect(dtls_context, &dst);
    ...
    printf("\nDebug:dtls_connect(dtls_context, &dst): %d\n",ret);
    fp = fopen("f1.txt", "r");
    if (fp == NULL)
    {
  ...
  //read from file
     ...
     // FD_SET(activeFD, &rfds);

        //FD_SET(fd, &wfds);


        if(fileno(fp)>fd)       activeFD = fileno(fp);
        ...
        result = select(activeFD+1, &rfds, &wfds, 0, &timeout); //select() uses a timeout that is a struct timeval (with seconds and microseconds)
    printf("\nfp:%d \t fd:%d \t ActiveFD=%d\n",fileno(fp),fd,fileno(fp));
      ...
      if (result < 0) {		/* error */
          ...
          if (FD_ISSET(fd, &wfds))
              ...
              printf("\nClient : (FD_ISSET(fd, &rfds) : dtls_handle_read(dtls_context)\n");
              dtls_handle_read(dtls_context);
          }

          else if (FD_ISSET(fileno(stdin), &rfds)) {
              printf("\nClient : FD_ISSET(fileno(stdin), &rfds) :handle_stdin() \n");
              handle_stdin();
          }

           /* Added by simpy */
          else if (FD_ISSET(fileno(fp), &rfds)!=NULL) {
                printf("\n\Read from my file\n");
              if (fgets(buf, 10, fp) ){
                  len += strlen(buf + len);
      ...
      //printf("\nAfter 2 : FD_ISSET(fp, &rfds) :read_file() %s  and len =%d\n",buf,len);
      ...
      printf("\nAfter 3: FD_ISSET(fp, &rfds) :read_file() %s  and len = %d\n",buf,len);
      //
  }
    fclose(fp);
    dtls_free_context(dtls_context);
